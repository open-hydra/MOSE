...
function print_usage {
  ...
  echo
  echo "Solver input"
  echo " <mach>                -->     reference Mach number for the test (0.1, 0.01, 0.001)"
  echo
  echo "Solver options"
...
if [[ $1 == solve ]]; then
  if [[ $# -lt 2 ]]; then
    echo "Error: Missing Mach number argument"
    print_usage
  fi
  if [[ $2 != 0.1 && $2 != 0.01 && $2 != 0.001 ]]; then
    echo "Error: Invalid Mach number. Allowed values are 0.1, 0.01, or 0.001"
    print_usage
  fi
  cd INPUT
  ln -sf ../../../../common/Air/phase.txt phase.txt
  ln -sf ../../../../common/Air/thermo.dat thermo.dat
  ln -sf ic-$2.tec ic.tec
  cd ..
  mkdir -p OUTPUT bin
  ulimit -s unlimited
