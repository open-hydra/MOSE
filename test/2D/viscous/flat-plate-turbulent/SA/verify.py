"""
Turbulent flat-plate skin-friction validation — SA.
...
MODEL        = "SA"
REF_MATCH    = ["CFL3D", "level 1"]     # substrings (all must be in the zone label)
TOLERANCE    = 5.0          # percent RMS relative error
...
try:
    ...
    ax.plot(x_m, Cf_m, "bd-", ms=12, markevery=10, label=f"MOSE", color="#0072B2", linewidth=4.0)
    ax.plot(x_r[x_r > 0], cf_r[x_r > 0], label="CFL3D", color="#E69F00", linestyle="--", linewidth=4.0)
