...
contains
  ...
  subroutine Riemann_LMRoe(dl,ul,vl,wl,pl,al,dltot,dr,ur,vr,wr,pr,ar,drtot,beta,url,urr,nx,ny,nz,F_r,F_u,F_v,F_w,F_E)
    ...
    ! A floor is retained (as in SU2's LMRoe/L2Roe) to keep a minimum of
    ...
    ! The floor is the shared low-Mach cutoff Mach (riemann-options-Mco): here it
    ! enters linearly (phi_min = Mco), since the LMRoe acoustic dissipation
    ! coefficient is D ~ phi. Pick Mco ~ a few x the flow's peak Mach.
    if (aRoe > 0.d0) then
